...
void Lexan::setupLexemes() {
    ...
    if (!inPi.is_open()) throw logic_error("Predeclared identifiers file doesn't open");
...
bool Lexan::match(const Token &token, int relPos) const{
    ...
    else return (token.type == nullToken.type && token.lex == nullToken.lex);
}

void Lexan::eat(Token const& token) {
    if (!match(token))
        throw logic_error("Wrong token: " + peek().lex + " (" + Token::tokenTypes[(int)peek().type]
                          + "), expected " + token.lex + " (" + Token::tokenTypes[(int)token.type] + ")");
    next();
}
...
bool Lexan::matchType(TOKEN_TYPE type, int relPos) const {
    ...
    else return (type == nullToken.type);
}

void Lexan::eatType(TOKEN_TYPE type) {
    if (!matchType(type))
        throw logic_error("Wrong token: " + tokenStream[curToken].lex + " (" + Token::tokenTypes[(int)tokenStream[curToken].type]
                          + "), expected (" + Token::tokenTypes[(int)type] + ")");
    next();
}
